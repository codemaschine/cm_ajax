...
/* Modified Tx_Fluid_ViewHelpers_Be_Buttons_IconViewHelper - by J. Dinse  */
 ...
 *
 */
class Tx_Carty_ViewHelpers_Be_Buttons_IconViewHelper extends Tx_Fluid_ViewHelpers_Be_AbstractBackendViewHelper {

	/**
	 * @var array allowed icons to be used with this view helper
	 */
	protected $allowedIcons = array('add', 'add_workspace', 'button_down', 'button_hide', 'button_left', 'button_unhide', 'button_right', 'button_up', 'clear_cache', 'clip_copy', 'clip_cut', 'clip_pasteafter', 'closedok', 'datepicker', 'deletedok', 'delete_record', 'edit2', 'garbage', 'helpbubble', 'icon_fatalerror', 'icon_note', 'icon_ok', 'icon_warning', 'new_el', 'options', 'perm', 'refresh_n', 'saveandclosedok', 'savedok', 'savedoknew', 'savedokshow', 'viewdok', 'zoom');
	 ...
	 * @param string $onclick Onclick eventhandler
	 * @param array $additionalAttributes Additional attributes of the resulting link
	 * @return string the rendered icon link
	 */
	public function render($uri, $icon = 'closedok', $title = '', $onclick = '', $additionalAttributes = array()) {
		...
		$additionalAttributesStr = empty($onclick) ? " onclick=\"$onclick\"" : '';
		foreach($additionalAttributes as $key => $value)
		  $additionalAttributesStr .= " $key=\"$value\"";
		
		return '<a href="' . $uri . '"'. $additionalAttributesStr .'><img' . $skinnedIcon . ' title="' . htmlspecialchars($title) . '" alt="" /></a>';
	}
}
?>
