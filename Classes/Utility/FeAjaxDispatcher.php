...
//$ajax['extensionName'] = 'CmAjax';
         
/**
 * @var $TSFE \TYPO3\CMS\Frontend\Controller\TypoScriptFrontendController
 */
$TSFE = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('TYPO3\\CMS\\Frontend\\Controller\\TypoScriptFrontendController', $TYPO3_CONF_VARS, $pid, 0);
...
$TSFE->initTemplate();
$TSFE->getConfigArray();
\TYPO3\CMS\Core\Core\Bootstrap::getInstance();  //\TYPO3\CMS\Core\Core\Bootstrap::getInstance()->loadConfigurationAndInitialize();
...
//$contentObject = isset($this->cObj) ? $this->cObj :
$TSFE->cObj->data['uid'] = intval(t3lib_div::_GP('cUid'));
$TSFE->cObj->data['pages'] = intval(t3lib_div::_GP('storagePages')); 
$configurationManager->setContentObject($TSFE->cObj);
...
$request->setFormat($ajax['format'] ? $ajax['format'] : 'html');


$arg_key = 'tx_'.strtolower($ajax['extensionName']).'_'.strtolower($ajax['pluginName']);

$getvars = $_GET['arguments'] ? $_GET['arguments'] : array();
$postvars = $_POST && $_POST[$arg_key] ? $_POST[$arg_key] : array();


if ($postvars['__trustedProperties'])
	unset($postvars['__trustedProperties']);

$request->setArguments(array_merge($getvars, $postvars));
